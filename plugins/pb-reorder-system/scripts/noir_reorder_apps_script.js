/***************************************************
 * Noir Reorder 자동화 스크립트 (Google Apps Script)
 * 브랜드: 노어
 *
 * 기능:
 ...
 *
 * 변경사항 (v1.5):
 *   - 쿼리 v1.2 컬럼 변경 반영: reorder_qty Y열(24) → AA열(26)
 ...
 *
 * 변경사항 (v1.2):
 *   - vendor_category 컬럼 추가 (B열)
 *   - 발주(리오더)_동대문 직접 기입 제거 → 5.2 리오더 내역만 사용
 *   - reorder_qty 컬럼 인덱스: X열(23) → Y열(24)
 *
 * 작성: 2025-12-10
 ***************************************************/
...
var CONFIG = {
  ...
  // 컬럼 인덱스 (0-based)
  // [v1.7] A~AA: 쿼리 출력 (27개), AB~AD: 시트에서 추가 (3개) = 총 30개
  ...
  RAW_REORDER: {
    ...
    REORDER_QTY: 29        // AD열: 리오더 추천 수량 (시트에서 계산, v1.7 변경)
  },
  ...
  // 시트 URL
  SHEET_URL: 'https://docs.google.com/spreadsheets/d/1q0CsxQXe3W-Y6xDcgNY-uT1oBDH_IZgrK-Gm7uTl8YE/edit?gid=1291961194#gid=1291961194'
};
 ...
 * ※ 평일(월~금)에만 실행, 주말(토/일)은 스킵
 */
function runNoirReorderProcess() {
  try {
    var today = new Date();
    ...
    Logger.log('[시작] 노어 리오더 프로세스 (' + todayStr + ')');

    // Step 0: 쿼리 실행 날짜 검증 (v1.6)
    ...
    // Step 4: Slack 알림 (v1.6: 데이터 날짜 포함)
    postSlackNotification(ss, todayStr, reorderItems, queryDateInfo);

    Logger.log('[완료] 노어 리오더 프로세스 완료 (' + reorderItems.length + '건 처리)');

  } catch (e) {
    console.error('runNoirReorderProcess FAILED:', e && e.stack || e);
    throw e;
  }
}

// ===================== Step 0: 쿼리 날짜 검증 (v1.6) =====================
...
function postFallbackSlackNotification(todayStr, queryDateStr) {
  try {
    ...
    var warningText = ':warning: *노어 리오더 데이터 갱신 필요*\n\n' +
      '━━━━━━━━━━━━━━━━━━━━━━\n' +
      '• 오늘 날짜: *' + todayStr + '*\n' +
    ...
    var blocks = [
      {
        type: 'header',
        text: { type: 'plain_text', text: '⚠️ 노어 리오더 - 데이터 갱신 필요' }
      },
      {
        type: 'section',
    ...
    chatPostMessageOrThrow({
      ...
      text: '노어 리오더 데이터 갱신 필요 (' + todayStr + ')',
      blocks: blocks
    });
 ...
 * 예: 'NR25F001_BLACK_FREE' → 'NR25F001'
 *     'D2_VD25S001_WHITE_M' → 'VD25S001'
 */
 ...
 * Y열(reorder_qty) > 0인 항목만 (vendor_category 추가로 X->Y로 밀림)
 * [v1.4] Price Down 상품(E/F등급, 할인이력) 제외
 */
function getReorderItems(ss) {
  ...
  for (var r = 1; r < data.length; r++) {
    ...
    if (reorderQty !== null && reorderQty > 0) {
      ...
      items.push({
        rowData: row.slice(0, 30),  // A~AD (30개 컬럼, v1.7 seasonality 컬럼 추가)
 ...
 * [v1.6] queryDateInfo 파라미터 추가 - 데이터 날짜 표시
 */
...
function buildSlackBlocks(todayStr, itemCount, totalQty, queryDateInfo) {
  ...
  return [
    {
      type: 'header',
      text: { type: 'plain_text', text: '노어 리오더 완료 (' + todayStr + ')' }
    },
    {
      type: 'section',
...
function buildMentionTargets() {
  ...
  return '<!subteam^' + CONFIG.SLACK_SUBTEAM_ID + '> ' + userMentions;
}
...
function testAppendToReorderLogSheet() {
  ...
  // 테스트용 더미 데이터 2건
  var testItems = [
    { productCode: 'NR_TEST_001', vendorCategory: '동대문', reorderQty: 10 },
    { productCode: 'NR_TEST_002', vendorCategory: '해외', reorderQty: 15 }
  ];
...
function testArchiveReorderItems() {
  ...
  dummyRowData[0] = 'NR_TEST_001';  // A열: 품번
  dummyRowData[1] = '동대문';        // B열: vendor_category

  var testItems = [
    { rowData: dummyRowData, productCode: 'NR_TEST_001', vendorCategory: '동대문', reorderQty: 10 }
  ];
...
function testSlackNotification() {
  ...
  // 테스트용 더미 데이터
  var testItems = [
    { productCode: 'NR_TEST_001', reorderQty: 10 },
    { productCode: 'NR_TEST_002', reorderQty: 15 }
  ];
