/***************************************************
 * Verda Reorder 자동화 스크립트 (Google Apps Script)
 * 브랜드: 베르다
 *
 * 기능:
...
var CONFIG = {
  ...
  // 시트 URL
  SHEET_URL: 'https://docs.google.com/spreadsheets/d/1Z8S1gl1bjc0YaQmIQU3_QhvpA8xxCEYHtedMuOM2wco'
};
 ...
 */
function runVerdaReorderProcess() {
  try {
    var today = new Date();
    ...
    Logger.log('[시작] 베르다 리오더 프로세스 (' + todayStr + ')');
    ...
    Logger.log('[완료] 베르다 리오더 프로세스 완료 (' + reorderItems.length + '건 처리)');

  } catch (e) {
    console.error('runVerdaReorderProcess FAILED:', e && e.stack || e);
    throw e;
  }
}
...
function postFallbackSlackNotification(todayStr, queryDateStr) {
  ...
  try {
    ...
    var warningText = ':warning: *베르다 리오더 데이터 갱신 필요*\n\n' +
      '━━━━━━━━━━━━━━━━━━━━━━\n' +
      '• 오늘 날짜: *' + todayStr + '*\n' +
    ...
    var blocks = [
      {
        type: 'header',
        text: { type: 'plain_text', text: '⚠️ 베르다 리오더 - 데이터 갱신 필요' }
      },
      {
        type: 'section',
    ...
    chatPostMessageOrThrow({
      ...
      text: '베르다 리오더 데이터 갱신 필요 (' + todayStr + ')',
      blocks: blocks
    });
 ...
 * 예: 'VD25S001_BLACK_FREE' → 'VD25S001'
...
function buildSlackBlocks(todayStr, itemCount, totalQty, queryDateInfo) {
  ...
  return [
    {
      type: 'header',
      text: { type: 'plain_text', text: '베르다 리오더 완료 (' + todayStr + ')' }
    },
    {
      type: 'section',
...
function testAppendToReorderLogSheet() {
  ...
  // 테스트용 더미 데이터 2건
  var testItems = [
    { productCode: 'VD_TEST_001', vendorCategory: '동대문', reorderQty: 10 },
    { productCode: 'VD_TEST_002', vendorCategory: '해외', reorderQty: 15 }
  ];
...
function testArchiveReorderItems() {
  ...
  dummyRowData[0] = 'VD_TEST_001';  // A열: 품번
  dummyRowData[1] = '동대문';        // B열: vendor_category

  var testItems = [
    { rowData: dummyRowData, productCode: 'VD_TEST_001', vendorCategory: '동대문', reorderQty: 10 }
  ];
...
function testSlackNotification() {
  ...
  // 테스트용 더미 데이터
  var testItems = [
    { productCode: 'VD_TEST_001', reorderQty: 10 },
    { productCode: 'VD_TEST_002', reorderQty: 15 }
  ];
