-- ##################################################################################
-- # 퀸즈셀렉션(Queens Selection) 리오더 최적화 분석 v1.2
-- # 가용 재고 기반 (Available Stock = Physical + In-Transit - Reserved)
-- #
-- # 변경사항 (from v1.1):
-- #   1. Monte Carlo 시뮬레이션 데이터 풀을 전체 앱 상품으로 확장
-- #      - 기존: 퀸즈셀렉션 브랜드 SKU만으로 컨텍스트 통계 계산 (샘플 부족 → Heuristic 폴백)
-- #      - 개선: 전체 앱 상품의 판매 데이터로 컨텍스트 통계 계산 (충분한 샘플 → Statistical)
-- #   2. category_spv_benchmark도 전체 앱 데이터 기반으로 계산
-- #   3. 최종 리오더 대상만 퀸즈셀렉션 브랜드 SKU로 필터링
-- #
-- # 유지사항 (from v1.1):
-- #   - 리오더 차수 계산 (daily_inventory_transaction 기반)
-- #   - 예측 기간 차등 적용: 초도~4차 14일, 5차+ 21일
-- #   - 수량 상한: 초도~4차 최대 10장 (예외: High-SPV & 일1개+)
-- #   - 시즌 정의: FW 9~2월, SS 3~8월 (시즌마다 차수 리셋)
-- #
...
DECLARE BRAND_CODE STRING DEFAULT 'QU';  -- 퀸즈셀렉션 브랜드
...
-- [NEW v1.2] 전체 앱 상품 (Monte Carlo 시뮬레이션용)
-- 브랜드 무관하게 전체 앱의 상품 판매 데이터를 사용
all_app_products AS (
  SELECT
    p.id AS product_id,
    p.created_at,
...
-- [NEW v1.2] 전체 앱 일별 판매 데이터 (Monte Carlo용)
...
all_app_daily_sales AS (
  SELECT
    c.calendar_date AS dt,
    p.product_id AS item_id,
    IFNULL(s.quantity, 0) AS quantity
  FROM (SELECT DISTINCT product_id FROM all_app_products) p
  CROSS JOIN (
...
-- 리오더 가능 품번 목록 (queens_master 기준) + vendor_category 포함
reorderable_products AS (
  SELECT DISTINCT
    barcode AS product_item_code,
    vendor_category
  FROM `damoa-lake.pb2_owned.queens_master`
  WHERE barcode IS NOT NULL AND barcode != ''
),

-- 퀸즈셀렉션 브랜드 실시간 재고 (stockDetail_raw 사용, queens_master에 있는 품번만)
queens_physical_stock AS (
  SELECT
...
reorder_history AS (
  ...
  WHERE inbound > 0
    ...
    AND barcode LIKE 'QU%'  -- 퀸즈셀렉션 브랜드만
  GROUP BY barcode
),
...
target_products AS (
  ...
  WHERE pi.product_item_code IN (SELECT mall_product_code FROM queens_physical_stock)
),
...
current_product_status AS (
  WITH sku_recent_performance AS (
    ...
    WHERE o.dt >= DATE_SUB(CURRENT_DATE(), INTERVAL 13 DAY)
      ...
      AND pi.product_item_code IN (SELECT mall_product_code FROM queens_physical_stock)
    GROUP BY 1
  )
  SELECT
  ...
  FROM queens_physical_stock stock
...
current_product_status_longterm AS (
  ...
  WHERE o.dt >= DATE_SUB(CURRENT_DATE(), INTERVAL 179 DAY)
    AND o.purchase_state <> 'WAIT'
    AND s.product_item_code IN (SELECT mall_product_code FROM queens_physical_stock)
  GROUP BY 1
),

-- [NEW v1.2] 카테고리 SPV 벤치마크 (전체 앱 데이터 기반)
category_spv_benchmark AS (
  ...
  JOIN all_app_products p ON s.item_id = p.product_id
  WHERE s.vcnt > 0
  GROUP BY 1
),

-- ================================================================================
-- 계층 2: 예측 엔진 (전체 앱 데이터 기반 Monte Carlo)
-- ================================================================================

-- [NEW v1.2] 14일 예측용 Monte Carlo (전체 앱 데이터 사용)
statistical_forecasts_14d AS (
  WITH historical_performance_with_context AS (
    SELECT
      ...
      s.item_id
    FROM all_app_daily_sales s  -- [CHANGED] 전체 앱 데이터 사용
    JOIN all_app_products p ON s.item_id = p.product_id
  ),
...
-- [NEW v1.2] 21일 예측용 Monte Carlo (전체 앱 데이터 사용)
