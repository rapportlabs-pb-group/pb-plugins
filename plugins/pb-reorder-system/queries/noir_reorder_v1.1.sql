-- ##################################################################################
-- # 노어(Noir) 리오더 최적화 분석 v1.1
-- # 가용 재고 기반 (Available Stock = Physical + In-Transit - Reserved)
-- #
-- # 변경사항 (from v1.0):
-- #   1. Reserved 쿼리 통합 (3일+ 예약 배송만)
-- #   2. SPV Threshold: 신상품 0.30, 기존상품 0.65 (Option B 채택)
-- #   3. delivery_state 필터 추가 (AI 합의: 이중 차감 방지)
-- #   4. 가용 재고 계산 BigQuery 내 통합
-- #   5. noor_master 테이블 필터 추가 (리오더 가능 품번만)
-- #
...
DECLARE RELATIVE_SPV_THRESHOLD_NEW FLOAT64 DEFAULT 0.30;  -- 신상품용 (Option B)
...
-- ================================================================================
-- [NEW] Reserved 수량 계산 (예약 배송 3일+ 후 출고만)
-- AI 토론 결과 반영: delivery_state 필터, ROW_NUMBER dedupe
-- ================================================================================
  ...
  FROM (
    SELECT
      ...
      ROW_NUMBER() OVER (
        ...
        ORDER BY cdc_timestamp DESC  -- updated_at/created_at 대신 cdc_timestamp 사용
      ) AS rn
...
current_product_status_30d AS (
  ...
  SELECT
    ...
    stock.vendor_category,  -- [NEW] vendor_category 추가
    IFNULL(p.base_category_name, 'No Category') AS base_category_name,
    stock.physical_stock,
    IFNULL(r.reserved_qty, 0) AS reserved_qty,  -- [NEW] Reserved 추가
    CASE
  ...
  LEFT JOIN reserved_qty_by_sku r ON stock.mall_product_code = r.mall_product_code  -- [NEW]
...
combined_forecasts AS (
  SELECT
    ...
    s30.vendor_category,  -- [NEW] vendor_category 전달
...
SELECT
  c.mall_product_code,
  c.vendor_category,  -- [NEW] vendor_category (동대문, 해외 등)
  c.base_category_name,
  c.physical_stock,
  c.reserved_qty,  -- [NEW] 예약 배송 수량 (3일+ 후 출고)
  c.product_type,
  c.decision_isoweek,
  c.sales_past_7_days,
...
WHERE
  -- [NEW] SPV Threshold Option B: 신상품 0.30, 기존상품 0.65
