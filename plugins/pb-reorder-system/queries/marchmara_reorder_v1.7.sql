-- ##################################################################################
-- # 마치마라(Marchmara) 리오더 최적화 분석 v1.7
...
DECLARE BRAND_CODE STRING DEFAULT 'MM';
...
-- [CHANGED v1.8] 당시즌 + 직전 시즌 품번 포함
reorderable_products AS (
  SELECT DISTINCT
    barcode AS product_item_code,
    vendor_category
  FROM `damoa-lake.pb1_owned.marchmara_master`
...
target_products AS (
  ...
  WHERE pi.product_item_code IN (SELECT mall_product_code FROM marchmara_physical_stock)
),
...
current_product_status AS (
  WITH sku_recent_performance AS (
    ...
    WHERE o.dt >= DATE_SUB(CURRENT_DATE(), INTERVAL 13 DAY)
      ...
      AND pi.product_item_code IN (SELECT mall_product_code FROM marchmara_physical_stock)
    GROUP BY 1
  )
  SELECT
  ...
  FROM marchmara_physical_stock stock
...
current_product_status_longterm AS (
  ...
  WHERE o.dt >= DATE_SUB(CURRENT_DATE(), INTERVAL 179 DAY)
    AND o.purchase_state <> 'WAIT'
    AND s.product_item_code IN (SELECT mall_product_code FROM marchmara_physical_stock)
  GROUP BY 1
),
