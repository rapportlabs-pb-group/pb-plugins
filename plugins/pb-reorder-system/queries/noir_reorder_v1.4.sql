...
-- [UPDATED v1.4] 카테고리 SPV 벤치마크 (동일 카테고리 상품 데이터 기반)
...
-- [UPDATED v1.4] 14일 예측용 Monte Carlo (동일 카테고리 상품 데이터 사용)
statistical_forecasts_14d AS (
  WITH historical_performance_with_context AS (
    SELECT
      ...
      s.item_id
    FROM category_daily_sales s  -- [CHANGED v1.4] 동일 카테고리 상품 데이터 사용
    JOIN category_matched_products p ON s.item_id = p.product_id
  ),
...
-- [UPDATED v1.4] 21일 예측용 Monte Carlo (동일 카테고리 상품 데이터 사용)
