-- ##################################################################################
-- # 퀸즈셀렉션(Queens Selection) 리오더 최적화 분석 v1.1
-- # 가용 재고 기반 (Available Stock = Physical + In-Transit - Reserved)
-- #
-- # 변경사항 (from v1.0):
-- #   1. 리오더 차수 계산 추가 (daily_inventory_transaction 기반)
-- #   2. 예측 기간 차등 적용: 초도~4차 14일, 5차+ 21일
-- #   3. 수량 상한: 초도~4차 최대 10장 (예외: High-SPV & 일1개+)
-- #   4. 시즌 정의: FW 9~2월, SS 3~8월 (시즌마다 차수 리셋)
-- #   5. Monte Carlo UDF: 14일, 21일용 추가
-- #
...
-- [NEW v1.1] 동적 시즌 계산 (F/W: 9월~2월, S/S: 3월~8월)
...
-- [NEW v1.1] 14일 예측용 Monte Carlo UDF
...
-- [NEW v1.1] 21일 예측용 Monte Carlo UDF
...
-- ================================================================================
-- [NEW v1.1] 리오더 차수 계산 (시즌 내 입고 이력 기반)
-- ================================================================================
reorder_history AS (
  SELECT
    barcode AS product_item_code,
...
current_product_status AS (
  ...
  LEFT JOIN reorder_history rh ON stock.mall_product_code = rh.product_item_code  -- [NEW]
...
-- ================================================================================
-- 계층 2: 예측 엔진 (14일/21일 차등 적용)
-- ================================================================================

-- [NEW v1.1] 14일 예측용 Monte Carlo
...
combined_forecasts AS (
  SELECT
    ...
    -- [NEW v1.1] 예측 기간 (초도~4차: 14일, 5차+: 21일)
    CASE
...
SELECT
  ...
  f.reserved_qty,
  f.product_type,
  f.reorder_order,  -- [NEW v1.1] 리오더 차수
  f.forecast_days,  -- [NEW v1.1] 예측 기간 (14/21일)
  f.decision_isoweek,
  f.sales_past_7_days,
  ...
  f.apply_qty_cap,  -- [NEW v1.1] 상한 적용 여부
