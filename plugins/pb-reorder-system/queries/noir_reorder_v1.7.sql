...
-- #   - 날짜 범위: 372일 (1년+1주) 동적 계산
-- #   - Context 사전 필터링 (Monte Carlo 성능 최적화)
-- #   - 카테고리 기반 필터링, 리오더 차수별 예측 기간 (14일/21일)
...
DECLARE BRAND_CODE STRING DEFAULT 'NR';
...
-- [NEW v1.8] 직전 시즌 시작일 (과년차 품번 누적 차수 계산용, 2개 시즌만)
DECLARE PREV_SEASON_START DATE DEFAULT (
  CASE
    WHEN CURRENT_MONTH >= 9 THEN DATE(CURRENT_YEAR, 3, 1)          -- 현재 F/W → 직전 S/S
    WHEN CURRENT_MONTH <= 2 THEN DATE(CURRENT_YEAR - 1, 3, 1)      -- 현재 F/W(1~2월) → 직전 S/S
    ELSE DATE(CURRENT_YEAR - 1, 9, 1)                               -- 현재 S/S → 직전 F/W
  END
);
...
reserved_qty_by_sku AS (
  ...
  WHERE
    ...
    AND ol.delivery_state NOT IN ('SHIPPED', 'DELIVERED', 'CANCELLED', 'COMPLETED')  -- v1.6: COMPLETED 추가
...
current_product_status AS (
  ...
  SELECT
    ...
    s.sku_spv_past_14d,
    -- [CHANGED v1.8] 동적 차수: 당시즌 품번=시즌 차수, 과년차=누적 차수
    CASE
...
final_forecasts AS (
  SELECT *,
    CAST(ROUND(CASE
      -- [CHANGED v1.7] High-SPV: 계절성 가중치 적용
      ...
      -- [CHANGED v1.7] Adaptive: 계절성 가중치 적용
...
SELECT
  ...
  -- [NEW v1.7] 계절성 디버깅 컬럼 (기존 인덱스 유지를 위해 맨 뒤 배치)
