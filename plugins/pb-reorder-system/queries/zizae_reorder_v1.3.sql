...
DECLARE BRAND_CODE STRING DEFAULT 'ZE';  -- 지재 브랜드
...
-- [NEW v1.3] 전체 앱 상품 (Monte Carlo 시뮬레이션용)
...
-- [NEW v1.3] 전체 앱 일별 판매 데이터 (Monte Carlo용)
...
-- 리오더 가능 품번 목록 (zizae_master 기준) + vendor_category 포함
...
reorderable_products AS (
  ...
  WHERE barcode IS NOT NULL AND barcode != ''
    -- 시즌 필터: 품번 3-4자리(년도) + 5자리(시즌코드) 기준
    AND (
      CASE
        -- FW 시즌 (9~12월): 현재 년도 FW 품번 (F, W, D)
...
-- 지재 브랜드 실시간 재고 (stockDetail_raw 사용, zizae_master에 있는 품번만)
...
-- [NEW v1.3] 카테고리 SPV 벤치마크 (전체 앱 데이터 기반)
...
-- [NEW v1.3] 14일 예측용 Monte Carlo (전체 앱 데이터 사용)
...
-- [NEW v1.3] 21일 예측용 Monte Carlo (전체 앱 데이터 사용)
