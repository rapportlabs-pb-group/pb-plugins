-- ##################################################################################
-- # 다나앤페타(Dana & Peta) 리오더 최적화 분석 v1.2
-- # 가용 재고 기반 (Available Stock = Physical + In-Transit - Reserved)
-- #
-- # 변경사항 (from v1.1):
-- #   1. Monte Carlo 시뮬레이션 데이터 풀 확장: 브랜드별 → 전체 앱 상품
-- #      - 기존: target_products (다나앤페타 브랜드만)
-- #      - 변경: all_app_products (전체 앱 상품 사용)
-- #   2. 충분한 샘플 수 확보로 Statistical 예측 정확도 향상
-- #      - 14개 이상 샘플 그룹 생성 확률 증가
-- #
...
-- [NEW v1.2] 전체 앱 일별 판매 (Monte Carlo용)
...
-- 브랜드별 일별 판매 (complete_daily_sales)
...
-- [MODIFIED v1.2] 카테고리 SPV 벤치마크 (전체 앱 기준)
...
-- [MODIFIED v1.2] 14일 예측용 Monte Carlo (전체 앱 데이터 풀 사용)
...
-- [MODIFIED v1.2] 21일 예측용 Monte Carlo (전체 앱 데이터 풀 사용)
