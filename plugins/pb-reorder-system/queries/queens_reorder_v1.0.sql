-- ##################################################################################
-- # 퀸즈셀렉션(Queens Selection) 리오더 최적화 분석 v1.0
-- # 가용 재고 기반 (Available Stock = Physical + In-Transit - Reserved)
-- #
-- # 기반: noir_reorder_v1.1.sql
-- # 변경사항:
-- #   1. BRAND_CODE: NR -> QS
-- #   2. master_table: queens_master 사용
-- #   3. product_item_code LIKE: NR% -> QS%
-- #
...
-- Note: queens_master JOIN으로 품번 필터링되므로 Brand 조건 불필요
...
reserved_qty_by_sku AS (
  ...
  WHERE
    -- 퀸즈셀렉션 브랜드만 (queens_master 품번 기준)
