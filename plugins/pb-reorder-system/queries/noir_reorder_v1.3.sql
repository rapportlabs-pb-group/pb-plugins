...
-- 리오더 가능 품번 목록 (noor_master 기준) + vendor_category 포함
reorderable_products AS (
  SELECT DISTINCT
    barcode AS product_item_code,
    vendor_category
  FROM `damoa-lake.pb2_owned.noor_master`
  WHERE barcode IS NOT NULL AND barcode != ''
),

-- 노어 브랜드 실시간 재고 (stockDetail_raw 사용, noor_master에 있는 품번만)
