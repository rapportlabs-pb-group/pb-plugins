-- ##################################################################################
-- # 지재(Zizae) 리오더 최적화 분석 v1.1
-- # 가용 재고 기반 (Available Stock = Physical + In-Transit - Reserved)
-- #
-- # 특징:
-- #   1. zizae_master 테이블 기준 리오더 가능 품번 필터
...
-- #   6. [v1.1] 시즌 필터: FW(9~2월) 해당 FW 품번, SS(3~8월) 해당 SS 품번만
-- #
...
DECLARE BRAND_CODE STRING DEFAULT 'ZE';  -- 지재 브랜드 (참고용, 실제 필터는 barcode 기반)
