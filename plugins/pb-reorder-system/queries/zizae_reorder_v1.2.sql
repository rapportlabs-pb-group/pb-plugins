...
-- #   6. [유지] 시즌 필터: FW(9~2월) 해당 FW 품번, SS(3~8월) 해당 SS 품번만
-- #
...
-- [NEW v1.2] 동적 시즌 계산 (F/W: 9월~2월, S/S: 3월~8월)
...
-- [NEW v1.2] 14일 예측용 Monte Carlo UDF
...
-- [NEW v1.2] 21일 예측용 Monte Carlo UDF
...
-- ================================================================================
-- [NEW v1.2] 리오더 차수 계산 (시즌 내 입고 이력 기반)
-- ================================================================================
reorder_history AS (
  SELECT
    barcode AS product_item_code,
...
combined_forecasts AS (
  SELECT
    ...
    -- [NEW v1.2] 예측 기간 (초도~4차: 14일, 5차+: 21일)
    CASE
...
SELECT
  ...
  f.reserved_qty,
  f.product_type,
  f.reorder_order,  -- [NEW v1.2] 리오더 차수
  f.forecast_days,  -- [NEW v1.2] 예측 기간 (14/21일)
  f.decision_isoweek,
  f.sales_past_7_days,
  ...
  f.apply_qty_cap,  -- [NEW v1.2] 상한 적용 여부
