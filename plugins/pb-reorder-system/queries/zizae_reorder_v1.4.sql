-- ##################################################################################
-- # 지재(Zizae) 리오더 최적화 분석 v1.4
-- # 가용 재고 기반 (Available Stock = Physical + In-Transit - Reserved)
-- #
-- # 변경사항 (from v1.3):
-- #   1. Monte Carlo 데이터 풀 최적화: 전체 앱 → 동일 카테고리 상품만
-- #      - 기존: 전체 앱 상품 (수십만개 × 날짜 = 데이터 폭발, 느린 쿼리)
-- #      - 개선: 브랜드 상품과 동일 카테고리의 앱 상품만 사용 (성능 최적화)
-- #   2. CTE 순서 재배치: target_products → brand_categories → category_matched_products
-- #
-- # 유지사항:
...
-- [NEW v1.4] 동일 카테고리의 전체 앱 상품 (Monte Carlo 시뮬레이션용)
-- 전체 앱이 아닌 브랜드와 동일 카테고리 상품만 사용하여 성능 최적화
...
-- [NEW v1.4] 카테고리 상품 일별 판매 (Monte Carlo용)
...
-- [MODIFIED v1.4] 카테고리 SPV 벤치마크 (동일 카테고리 상품 기준)
...
-- [MODIFIED v1.4] 14일 예측용 Monte Carlo (동일 카테고리 데이터 사용)
...
-- [MODIFIED v1.4] 21일 예측용 Monte Carlo (동일 카테고리 데이터 사용)
