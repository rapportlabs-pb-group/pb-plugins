...
-- [CHANGED v1.5] 동적 날짜 범위 (372일 = 1년 + 1주)
...
-- 동일 카테고리의 전체 앱 상품 (Monte Carlo 시뮬레이션용)
...
-- [NEW v1.5] 대상 상품의 Context Group 추출 (Monte Carlo 필터링용)
...
-- 카테고리 SPV 벤치마크 (동일 카테고리 상품 데이터 기반)
...
-- [UPDATED v1.5] 14일 예측용 Monte Carlo (Context 필터링 적용)
statistical_forecasts_14d AS (
  ...
  -- [UPDATED v1.5] 필요한 Context Group만 통계 계산
  context_group_stats AS (
    ...
    -- [NEW v1.5] Target Context Group 필터링
...
-- [UPDATED v1.5] 21일 예측용 Monte Carlo (Context 필터링 적용)
