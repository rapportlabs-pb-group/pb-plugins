/***************************************************
 * Dana & Peta Reorder 자동화 스크립트 (Google Apps Script)
 * 브랜드: 다나앤페타
 *
 * 기능:
 ...
 *   - 쿼리 실행일 검증 로직 추가 (당일 데이터 아니면 폴백 메시지)
...
var CONFIG = {
  ...
  // 컬럼 인덱스 (0-based)
  // [v1.3] A~AA: 쿼리 출력 (27개), AB~AD: 시트에서 추가 (3개) = 총 30개
  ...
  RAW_REORDER: {
    ...
    REORDER_QTY: 29        // AD열: 리오더 추천 수량 (시트에서 계산, v1.3 변경)
  },
  ...
  // 멘션 대상 (지재와 동일한 그룹 태그 사용 - 필요시 변경)
  SLACK_SUBTEAM_ID: 'S03VBFWTK40',
  SLACK_USER_IDS: ['U09F2ECCHB5', 'U09RA3B00SH'],  // 추가 멘션할 유저 ID

  // 시트 URL
  SHEET_URL: 'https://docs.google.com/spreadsheets/d/1JkTrLc7uhEfMpHIFUZ2EvwFy3Kp76AGa-h9pagPwz5k/edit?gid=483257257#gid=483257257'
};

// ===================== 쿼리 날짜 검증 (v1.2) =====================

/**
 * raw_reorder 시트의 query_executed_at 컬럼을 확인하여
 * 당일 데이터인지 검증
 * @returns {Object} { isToday: boolean, dateStr: string, fullDateTime: string }
 */
// ===================== 휴일 체크 =====================
...
function validateQueryExecutedDate(ss) {
  ...
  if (!sheet) {
    return { isToday: false, dateStr: '시트 없음', fullDateTime: '' };
  }

  var data = sheet.getDataRange().getValues();
  if (data.length < 2) {
    return { isToday: false, dateStr: '데이터 없음', fullDateTime: '' };
  }

  var COL_QUERY_DATE = CONFIG.RAW_REORDER.QUERY_EXECUTED_AT;
  var queryDateValue = data[1][COL_QUERY_DATE];  // 첫 번째 데이터 행

  // 날짜 파싱
  var queryDate;
  if (queryDateValue instanceof Date) {
    queryDate = queryDateValue;
  } else if (typeof queryDateValue === 'string' && queryDateValue.length > 0) {
    queryDate = new Date(queryDateValue);
  } else {
    return { isToday: false, dateStr: '날짜 없음', fullDateTime: '' };
  }

  if (isNaN(queryDate.getTime())) {
    return { isToday: false, dateStr: '파싱 실패', fullDateTime: '' };
  }
  ...
  var fullDateTime = Utilities.formatDate(queryDate, CONFIG.TIMEZONE, 'yyyy-MM-dd HH:mm:ss');
...
/**
 * 쿼리 데이터가 당일이 아닌 경우 폴백 Slack 메시지 발송
 */
function postFallbackSlackNotification(todayStr, queryDateStr) {
  try {
    ...
    var blocks = [
      {
        type: 'header',
        text: { type: 'plain_text', text: '⚠️ 다나앤페타 리오더 데이터 갱신 필요' }
      },
      {
        type: 'section',
        text: {
          ...
          text: '━━━━━━━━━━━━━━━━━━━━━━\n' +
            ...
            '<!subteam^' + CONFIG.SLACK_SUBTEAM_ID + '>' +
            (CONFIG.SLACK_USER_IDS && CONFIG.SLACK_USER_IDS.length > 0
              ? ' ' + CONFIG.SLACK_USER_IDS.map(function(uid) { return '<@' + uid + '>'; }).join(' ')
              : '')
        }
      }
    ];
    ...
    chatPostMessageOrThrow({
      ...
      text: '다나앤페타 리오더 데이터 갱신 필요 (' + todayStr + ')',
      blocks: blocks
    });

    Logger.log('[폴백] 데이터 갱신 필요 Slack 알림 발송');
...
/**
 * 전체 리오더 프로세스 실행
 * 1. 쿼리 실행일 검증 (당일 데이터 확인)
 * 2. 5.2 리오더 내역 탭에 전체 기록
 * 3. 아카이브 저장
 * 4. Slack 알림
 *
 * ※ 평일(월~금)에만 실행, 주말(토/일)은 스킵
 */
function runDanapetaReorderProcess() {
  try {
    var today = new Date();
    ...
    Logger.log('[정보] 쿼리 실행일: ' + queryDateInfo.fullDateTime + ', 오늘 날짜: ' + todayStr);

    if (!queryDateInfo.isToday) {
      Logger.log('[경고] raw_reorder 데이터가 오늘 날짜가 아닙니다. 폴백 메시지 발송.');
      postFallbackSlackNotification(todayStr, queryDateInfo.dateStr);
      return;
    }

    Logger.log('[시작] 다나앤페타 리오더 프로세스 (' + todayStr + ')');
    ...
    Logger.log('[완료] 다나앤페타 리오더 프로세스 완료 (' + reorderItems.length + '건 처리)');

  } catch (e) {
    console.error('runDanapetaReorderProcess FAILED:', e && e.stack || e);
    throw e;
  }
}
...
function getReorderItems(ss) {
  ...
  for (var r = 1; r < data.length; r++) {
    ...
    if (reorderQty !== null && reorderQty > 0) {
      items.push({
        rowData: row.slice(0, 30),  // A~AD (30개 컬럼, v1.3 seasonality 컬럼 추가)
...
function archiveReorderItems(ss, reorderItems, today) {
  ...
  if (!sheet) {
    ...
    var headers = [
      ...
      'conservative_forecast_60d', 'conservative_forecast_90d',
      ...
      'reorder_qty_conservative_60d', 'reorder_qty_conservative_90d'
    ];
 ...
 * @param {Object} queryDateInfo - 쿼리 실행일 정보 (v1.2)
 */
...
function buildSlackBlocks(todayStr, itemCount, totalQty, queryDateInfo) {
  ...
  // [v1.2] 데이터 날짜 정보 추가
  var dataDateLine = '';
  if (queryDateInfo && queryDateInfo.fullDateTime) {
    dataDateLine = '• 데이터 기준: `' + queryDateInfo.fullDateTime + '`\n';
  }
  ...
  return [
    {
      type: 'header',
      text: { type: 'plain_text', text: '다나앤페타 리오더 완료 (' + todayStr + ')' }
    },
    {
      type: 'section',
...
function testAppendToReorderLogSheet() {
  ...
  // 테스트용 더미 데이터 2건
  var testItems = [
    { productCode: 'DN_TEST_001', vendorCategory: '동대문', reorderQty: 10 },
    { productCode: 'DN_TEST_002', vendorCategory: '해외', reorderQty: 15 }
  ];
...
function testArchiveReorderItems() {
  ...
  // 테스트용 더미 데이터 (32개 컬럼: A~AF)
  var dummyRowData = new Array(32).fill('TEST');
  dummyRowData[0] = 'DN_TEST_001';  // A열: 품번
  dummyRowData[1] = '동대문';        // B열: vendor_category

  var testItems = [
    { rowData: dummyRowData, productCode: 'DN_TEST_001', vendorCategory: '동대문', reorderQty: 10 }
  ];
...
function testSlackNotification() {
  ...
  // 테스트용 더미 데이터
  var testItems = [
    { productCode: 'DN_TEST_001', reorderQty: 10 },
    { productCode: 'DN_TEST_002', reorderQty: 15 }
  ];
